<template>
  <div
    class="border-light dark:bg-dark-2 dark:border-dark-3 shadow-1 dark:shadow-card rounded-lg border bg-white px-4 py-4 sm:px-6 md:px-8 md:py-5"
  >
    <ul class="flex items-center">      <template v-for="(item, index) in breadcrumbItems" :key="index">
        <li class="flex items-center">
          <router-link v-if="item.href" :to="item.href" :class="getLinkClass(index)">
            {{ item.text }}
          </router-link>
          <span
...
// Nom du composant pour corriger l'erreur ESLint
defineOptions({
  name: 'BreadcrumbNav'
})

// Props pour recevoir les éléments de breadcrumb depuis le parent
